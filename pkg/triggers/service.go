...
func (svc *service) Run(triggerName string, vars map[string]interface{}) ([]ConditionResult, error) {
	...
	for i, condition := range t {
		...
		var whenResult bool
		log.Debugf("This is the version with the whenResult variable")
		...
		if whenResult {
			...
			log.Debugf("The OncePer condition will not be evaluated since the when condition evaluates to false")
		}
